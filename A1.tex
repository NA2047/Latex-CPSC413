...
\\
\\
The Graph I have given below is not the smallest number of nodes that you can use to give a counter example. Infact key to the counter example comes by denying the flaw with this algorithm like the one above comes from when there is a tie. particularly when there is no even Node to choose from.
\begin{center}
\begin{tikzpicture}[auto, node distance=2cm, every loop/.style={},
                    thick,main node/.style={circle,draw,font=\sffamily\Large\bfseries}]
